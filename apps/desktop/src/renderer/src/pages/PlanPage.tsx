...
function renderItemMeta(item: any) {
  if (!item) return null;

  if (item.kind === "BIBLE_PASSAGE" || item.kind === "BIBLE_VERSE") {
    const ref = item.reference || item.ref || item.title || "Passage biblique";
    const tr = item.translation ? String(item.translation) : "";
    const title = tr ? `${ref} (${tr})` : ref;
    ...
    return (
      <div style={{ display: "grid", gap: 4 }}>
        ...
        <div style={{ opacity: 0.7, fontSize: 13 }}>
          {blocks.length ? `${blocks.length} blocs` : "Texte vide"} {item.source ? `• source: ${String(item.source)}` : ""}
        </div>
      </div>
    );
  }

  const t = item.title || item.name || item.kind || "Item";
  return <div style={{ fontWeight: 900 }}>{String(t)}</div>;
}
